...
가상 머신 설치 시 OS를 `Other Linux 64bit`로 하지 말고, `Redhat 64bit`로 하면 훨씬 나아짐.
...
**`한국어`로 표시된 것을 선택**하면 `Shift+Space`로 한/영 전환이 된다. 즉, **`한국어(101/104키보드 호환)`으로 선택하지 말 것**.
...
화면 상단에 `ko`로 표시된 입력기를 클릭해서 `한국어(Hangul)`를 선택하면, `Shift+Space`로 한영 전환 가능
...
가상 머신 생성 후 가상 머신을 실행하기 전에 네트워크 설정을 구성해준다. 아래 내용은 가상 머신을 생성할 때마다 반복적으로 지정해줘야 한다. 가상 머신을 복제할 때는 `네트워크 카드 모두 초기화`에 체크하면 다시 지정해주지 않아도 된다.
...
NAT와 Host-only 모두 활성화 시킨 상태에서 인터넷 연결, 호스트 연결이 모두 잘 되는 상태에서의 `ifconfig -a` 결과는 다음과 같다.

```
3개
```

`route -n`의 결과는 다음과 같다.
