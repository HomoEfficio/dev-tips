...
Java와 C++의 차이를 통해 C++을 최대한 빨리 배워보자.
...
실행 결과는 다음과 같다. 참조형 변수에 재할당 시 얕은 복사가 발생하고 원래 값을 덮어씀을 알 수 있다.
...
참조형 변수에 다른 값을 할당할 때 할당 대신 얕은 복사가 일어나고 원래 값이 덮어써지는 현상은 상당히 낯설지만, 실제 개발할 때는 참조형 변수에 다른 값을 할당하지 않으면 되므로 현실적으로는 크게 신경쓸 필요가 없다. 
