...
공식 책의 내용을 참조하면 참조는 소유권이 없는 포인터(메모리 주소값을 가지는 변수)이고, 참조에 대한 참조를 생성하는 것을 borrow라고 한다.  

![](https://images2.imgbox.com/c3/2f/rUOodOyQ_o.png)

그러면 참조에 대한 참조를 생성하는 것을 왜 빌린다(borrow)라고 하는 걸까? 참조에 대한 참조, 구체적으로 참조1에 대한 참조2는 참조1이 가리키는 데이터에 대한 소유권을 가지지 못한다(그래서 위 캡처 제목처럼 Non-Owning Point라고 한다).  
