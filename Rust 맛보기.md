...
- 힙에 생성되는 변수를 다른 변수에 할당하면 Ownership은 복사되지 않고(Not Copy) 이동(Move)된다. 즉, **Owner는 언제나 1개다.**
    ...
    - 이 과정에서 1번 회수한 메모리를 2번째 해제하기 위해 다시 참조하는 순간 undefined behavior 가 일어나게 된다.
...
### Borrow라는 말은 왜 나오는 걸까?

공식 책의 내용을 참조하면 참조는 소유권이 없는 포인터(메모리 주소값을 가지는 변수)이고, (소유권이 있는) 변수에 대해 (소유권이 없는) 참조를 생성하는 것을 borrow라고 한다.  

![](https://images2.imgbox.com/c3/2f/rUOodOyQ_o.png)

그러면 왜 빌린다(borrow)는 표현을 쓰는 걸까? 소유권 있는 변수를 가리키는 참조, 구체적으로 소유권 있는 변수1에 대한 참조2는 변수1이 가리키는 데이터에 대한 소유권을 가지지 못한다(그래서 위 캡처 제목처럼 Non-Owning Pointer라고 한다).  
참조2는 변수1이 가리키는 데이터에 대한 소유권은 가지지 못 하지만, 변수1이 가리키는 데이터를 사용(읽을 수 있고, 규칙을 지킨다면 변경도 할 수 있다)할 수 있다. 따라서 변수1을 가리키는 참조2를 생성하는 것은 변수1이 가리키는 데이터에 대한 사용권을 빌리는 것이라고 표현할 수 있다.
